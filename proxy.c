...
DWORD WINAPI client_handler(LPVOID p) {
    ...
    // 将主要逻辑包裹在 try-except 块中，防止线程崩溃导致主程序退出
    __try {
    ...
    __except(EXCEPTION_EXECUTE_HANDLER) {
        log_msg("[Fatal] Client Thread Crashed (Code: 0x%08X)", GetExceptionCode());
    }
...
DWORD WINAPI server_thread(LPVOID p) {
    ...
    __except(EXCEPTION_EXECUTE_HANDLER) {
        log_msg("[Fatal] Server Thread Crashed (Code: 0x%08X)", GetExceptionCode());
        // 尝试重启服务或退出
        g_proxyRunning = FALSE;
    }
