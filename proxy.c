...
DWORD WINAPI client_handler(LPVOID p) {
    ...
    BOOL is_shadowsocks = (_stricmp(g_proxyConfig.type, "shadowsocks") == 0);
    ...
    if (is_vless) {
        ...
        // 注意: 标准SS需要在此处加密(AEAD)。此处实现的是 SS-over-TLS (Method: none/plain) 协议结构。
