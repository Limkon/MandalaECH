...
void parse_uuid(const char* uuid_str, unsigned char* out) {
    ...
    while (*p && i < 16) {
        ...
        int v;
        if (sscanf(p, "%2x", &v) == 1) { out[i++] = (unsigned char)v; p += 2; } else { p++; }
    }
}
...
void proxy_free(void* p, size_t size) {
    if (p) { free(p); InterlockedAdd64(&g_total_allocated_mem, -(LONG64)size); }
}
...
DWORD WINAPI client_handler(LPVOID p) {
    ...
    if (c_buf[0] == 0x05) { 
        ...
        // [Fix] 修复 HTTP 请求解析 (处理 http:// 前缀)
        if (sscanf(c_buf, "%15s %250s", method, host) == 2) {
            char *p_start = host;
            // 移除 http:// 或 https:// 前缀
            if (strstr(host, "http://") == host) p_start += 7;
            else if (strstr(host, "https://") == host) p_start += 8;
            
            char *p_col = strchr(p_start, ':');
            if (p_col) { 
                *p_col = 0; 
                port = atoi(p_col+1); 
            } else {
                if(stricmp(method, "CONNECT")==0) port = 443;
                else port = 80;
            }
            
            // 将纯 Host 移回开头
            if (p_start != host) memmove(host, p_start, strlen(p_start) + 1);
    ...
    if (getaddrinfo(localConfig.host, port_str, &hints, &res) != 0) goto cl_end;
    ...
    if (!strstr(ws_read_buf, "101")) goto cl_end; 
    ...
    if (is_vless) {
        ...
        // [Fix] SOCKS5 Outbound 修复: ws_read_payload_exact 增加 max_len 参数
        ...
        char resp_buf[512]; 
        if (ws_read_payload_exact(&tls, resp_buf, 2, sizeof(resp_buf)) < 2) goto cl_end;
        ...
        tls_write(&tls, ws_send_buf, flen);
        if (ws_read_payload_exact(&tls, resp_buf, 4, sizeof(resp_buf)) == 0) goto cl_end;
        if (resp_buf[1] != 0x00) goto cl_end;
    }
