...
cJSON* ParseMandala(const char* link); // 前向声明
...
// ... (LoadSettings, SaveSettings, SetAutorun, IsAutorun, ParseTags, GetUniqueTagName, ParseNodeConfigToGlobal, SwitchNode, DeleteNode, AddNodeToConfig 保持不变，请确保包含 SafeStrCpy 等修复) ...
// 为了节省篇幅，这里省略中间未修改的函数，请保留原有的实现。
// 重点是下面的解析函数和 BatchAdd 函数。

void LoadSettings() {
    // ... (保持原有代码，确保使用 snprintf 或 _snwprintf) ...
...
int Internal_BatchAddNodesFromText(const char* text, cJSON* outbounds) {
    ...
    // 如果整体解码失败（可能不是base64订阅），直接使用原文
    if (decoded && decLen > 0) sourceText = (char*)decoded; 
    else { sourceText = strdup(text); if (decoded) free(decoded); }
    if (!sourceText) return 0;
    
    // [Fix] 使用标准 strtok 替代 strtok_s，并处理 mixed line endings
    ...
    while (line) {
        ...
        if (strlen(line) > 0) {
            ...
            if (node) {
                ...
                // log_msg("Failed to parse node: %.50s...", line); // 可选调试
            }
        }
        line = strtok(NULL, "\r\n ,");
    }
...
// ... (ImportFromClipboard, UpdateAllSubscriptions, ToggleTrayIcon 保持不变) ...
...
cJSON* ParseSocks(const char* link) {
    ...
    // [Fix] 提取主机信息，正确处理无 path/query 的情况
    p = at + 1; 
    ...
    // 如果 URL 是 socks://... 且有 security=tls 或 sni，则视为 socks_tls
    if (enableTls) {
...
// ... (ParseShadowsocks, ParseVmess 保持不变) ...
...
cJSON* ParseVlessOrTrojan(const char* link) {
    ...
    // [Fix] 这里的 hostEnd 计算逻辑现在能正确处理无 # 的情况
    const char* hostEnd = colon;
    if (!hostEnd || (qMark && qMark < hostEnd)) hostEnd = qMark;
    if (!hostEnd || (hash && hash < hostEnd)) hostEnd = hash;
    if (!hostEnd) hostEnd = p + strlen(p);
