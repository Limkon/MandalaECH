...
static char* InternalHttpsGet(const char* url, BOOL useProxy) {
    ...
    if (useProxy) {
        ...
        if (n <= 0 || !strstr(buf, "200 Connection Established")) { closesocket(s); return NULL; }
    }
...
int BuildDNSQueryHTTPS(const char* domain, unsigned char* buf) {
    int pos = 0;
    // ID=0, Flags=0x0100 (RD), QDCOUNT=1
    buf[pos++] = 0; buf[pos++] = 0; 
...
char* Utils_HttpBytesGet(const char* url, int* out_len) {
    // 简单实现：为了简洁，复用 InternalHttpsGet 的逻辑但做微调
    // 这里为了代码精简，我们直接手动实现一个精简版，或者修改 InternalHttpsGet 支持 binary
    // 由于字数限制，我们这里直接复制 InternalHttpsGet 的逻辑并允许 binary 返回
    URL_COMPONENTS_SIMPLE u;
    if (!ParseUrl(url, &u)) { *out_len=0; return NULL; }
    
    // ... SSL Init (Same as above) ...
    static int ssl_inited = 0;
    ...
    if(!he || s == INVALID_SOCKET) { if(s!=INVALID_SOCKET) closesocket(s); return NULL; }
    ...
    if (SSL_connect(ssl) != 1) { SSL_shutdown(ssl); SSL_free(ssl); SSL_CTX_free(ctx); closesocket(s); return NULL; }

    char req[2048];
    snprintf(req, sizeof(req), "GET %s HTTP/1.1\r\nHost: %s\r\nAccept: application/dns-message\r\nConnection: close\r\n\r\n", u.path, u.host);
    SSL_write(ssl, req, (int)strlen(req));

    int buf_size = 65536; char* resp = (char*)malloc(buf_size); int total=0;
    while(1) {
        int r = SSL_read(ssl, resp+total, 4096);
        if (r<=0) break;
        total+=r;
        if (total+4096 > buf_size) { buf_size*=2; resp = realloc(resp, buf_size); }
    }
    ...
    char* res = NULL; *out_len = 0;
    if (body_idx != -1 && body_idx < total) {
        *out_len = total - body_idx;
        res = (char*)malloc(*out_len);
        memcpy(res, resp+body_idx, *out_len);
    }
    free(resp);
...
char* FetchECHFromDoH(const char* dohUrl, const char* sni) {
    ...
    if (!resp || resp_len < 12) { 
        log_msg("[ECH] DoH failed/empty"); if(resp) free(resp); return NULL; 
    }
    ...
    int ancount = (resp[6] << 8) | resp[7]; // Answer Count
    
    for (int i=0; i<ancount; i++) {
        if (pos >= resp_len) break;
        
        // [Fix] 跳过 Answer Name (支持 Pointer 0xC0xx)
        ...
        pos += 10; // Skip TYPE, CLASS, TTL, RDLEN header
        
        if (pos + rdlen > resp_len) break; // Bounds check
        
        if (type == 65) { // HTTPS Record
            unsigned char* rdata = resp + pos;
            int rpos = 2; // Skip Priority (2 bytes)
            
            // [Fix] 健壮的 TargetName 跳过逻辑
            // HTTPS 的 TargetName 是 wire format，如果不是 root(.), 必须跳过 labels
            if (rpos < rdlen && rdata[rpos] == 0) {
                rpos++; // Root domain (.)
            } else {
                while (rpos < rdlen && rdata[rpos] != 0) {
                    rpos += rdata[rpos] + 1;
                }
                rpos++; // [Critical Fix] 必须跳过最后的 0x00 结束符
            }
            
            // Parse Params: Key(2) + Len(2) + Value(...)
            while (rpos + 4 <= rdlen) {
                int key = (rdata[rpos] << 8) | rdata[rpos+1];
                int vlen = (rdata[rpos+2] << 8) | rdata[rpos+3];
                rpos += 4;
                
                if (rpos + vlen > rdlen) break; // Overflow check
                
                if (key == 5) { // ECH Config param
