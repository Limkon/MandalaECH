...
static char* InternalHttpsGet(const char* url, BOOL useProxy, int* out_len_binary) {
    ...
    if (useProxy) {
        ...
        if (n <= 0) { 
            log_msg("[Utils] Proxy handshake failed (recv)"); 
            closesocket(s); 
            return NULL; 
        }
        buf[n] = 0;
...
// 供内部调用的二进制获取版本 (DoH 专用)
char* Utils_HttpBytesGet(const char* url, int* out_len) {
    if (!url) return NULL;
    // DoH 强制直连 (FALSE)，避免循环依赖；或者你可以改为根据配置决定
...
char* FetchECHFromDoH(const char* dohUrl, const char* sni) {
    ...
    for (int i = 0; i < ancount; i++) {
        ...
        if (type == 65) { // HTTPS Record
            ...
            // HTTPS 的 TargetName 是 wire format
