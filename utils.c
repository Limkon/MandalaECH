...
void log_msg(const char *format, ...) {
    ...
    if (h && IsWindow(h)) {
        ...
        if (wBuf) {
            MultiByteToWideChar(CP_UTF8, 0, final_msg, -1, wBuf, wLen); wBuf[wLen] = 0;
            PostMessageW(h, WM_LOG_UPDATE, 0, (LPARAM)wBuf);
        }
    }
}

void log_wsa_error(const char* context) {
    log_msg("[Error] %s Failed. Code: %d", context, WSAGetLastError());
}
...
char* GetClipboardText() {
    ...
    if (hData) {
        ...
        if (pszTextW) {
            ...
            text[utf8Len] = 0; TrimString(text); GlobalUnlock(hData); CloseClipboard(); return text;
        }
    }
    ...
    if (hData) {
        ...
        if (pszText) {
            ...
            TrimString(text); GlobalUnlock(hData); CloseClipboard(); return text;
        }
    }
    ...
    *out_len = (len * 3) / 4;
    ...
    while (i < len) {
        ...
        while (i < len && cnt < 4) {
            ...
            if (strchr("\r\n ", c)) { i++; continue; }
            if (c == '=') { i++; break; }
            int v = GetBase64Val(c); if (v == -1) { free(out); return NULL; }
            vals[cnt++] = v; i++;
        }
        if (cnt > 0) {
            uint32_t t = (vals[0] << 18) | ((cnt > 1 ? vals[1] : 0) << 12) | ((cnt > 2 ? vals[2] : 0) << 6) | ((cnt > 3 ? vals[3] : 0));
            if (j < *out_len) out[j++] = (t >> 16) & 0xFF;
            if (cnt > 2 && j < *out_len) out[j++] = (t >> 8) & 0xFF;
            if (cnt > 3 && j < *out_len) out[j++] = t & 0xFF;
        }
    }
...
void Base64Encode(const unsigned char* src, int len, char* dst) {
    ...
    for (; i < len; i += 3) {
        ...
        dst[j++] = base64_chars[(v >> 12) & 0x3F];
        dst[j++] = (i + 1 < len) ? base64_chars[(v >> 6) & 0x3F] : '=';
        dst[j++] = (i + 2 < len) ? base64_chars[v & 0x3F] : '=';
    }
    dst[j] = 0;
}
...
void Base64UrlEncode(const unsigned char* src, int len, char* dst) {
    ...
    for (; i < len; i += 3) {
        ...
        dst[j++] = tbl[(v >> 18) & 0x3F];
        dst[j++] = tbl[(v >> 12) & 0x3F];
        dst[j++] = (i + 1 < len) ? tbl[(v >> 6) & 0x3F] : '=';
        dst[j++] = (i + 2 < len) ? tbl[v & 0x3F] : '=';
    }
    while (j > 0 && dst[j - 1] == '=') j--;
    dst[j] = 0;
}
...
static BOOL ParseUrl(const char* url, URL_COMP* out) {
    memset(out, 0, sizeof(URL_COMP));
    const char* p = url;
    if (!strncmp(p, "https://", 8)) { out->port = 443; p += 8; }
    else if (!strncmp(p, "http://", 7)) { out->port = 80; p += 7; }
    else return FALSE;
    const char* sl = strchr(p, '/');
    int hLen = sl ? (int)(sl - p) : (int)strlen(p);
    if (hLen >= 256) return FALSE;
    strncpy(out->host, p, hLen);
    char* col = strchr(out->host, ':'); if (col) { *col = 0; out->port = atoi(col + 1); }
    if (sl) strcpy(out->path, sl); else strcpy(out->path, "/");
    return TRUE;
}

static char* InternalHttpsGet(const char* url, BOOL useProxy, int* out_len) {
    URL_COMP u; if (!ParseUrl(url, &u)) return NULL;
    if (out_len) *out_len = 0;
    
    static int ini = 0; if (!ini) { SSL_library_init(); OpenSSL_add_all_algorithms(); SSL_load_error_strings(); ini = 1; }

    const char* tHost = useProxy ? "127.0.0.1" : u.host;
    int tPort = useProxy ? g_localPort : u.port;
    ...
    DWORD tv = 10000; setsockopt(s, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv, 4); setsockopt(s, SOL_SOCKET, SO_SNDTIMEO, (char*)&tv, 4);

    if (connect(s, (struct sockaddr*)&a, sizeof(a))) { closesocket(s); return NULL; }

    if (useProxy) {
        char req[512], buf[1024];
        snprintf(req, 512, "CONNECT %s:%d HTTP/1.1\r\nHost: %s:%d\r\n\r\n", u.host, u.port, u.host, u.port);
        send(s, req, strlen(req), 0);
        if (recv(s, buf, 1023, 0) <= 0 || !strstr(buf, "200 Connection Established")) { closesocket(s); return NULL; }
    }
    ...
    char req[4096];
    const char* acc = out_len ? "application/dns-message" : "*/*";
    snprintf(req, 4096, "GET %s HTTP/1.1\r\nHost: %s\r\nUser-Agent: Mandala/1.0\r\nAccept: %s\r\nConnection: close\r\n\r\n", u.path, u.host, acc);
    SSL_write(ssl, req, strlen(req));

    int bSize = 65536, tRead = 0;
    char* resp = (char*)malloc(bSize);
    while (1) {
        if (tRead + 4096 >= bSize) { bSize *= 2; resp = realloc(resp, bSize); }
        int r = SSL_read(ssl, resp + tRead, 4096);
        if (r <= 0) break;
        tRead += r;
    }
    resp[tRead] = 0;
    ...
    // [Check 200 OK]
    if (!strstr(resp, "HTTP/1.1 200") && !strstr(resp, "HTTP/2 200")) {
    ...
    if (body) {
        body += (body[0] == '\r' ? 4 : 2);
        int len = tRead - (int)(body - resp);
        if (len > 0) {
            res = (char*)malloc(len + 1); memcpy(res, body, len); res[len] = 0;
...
char* Utils_HttpGet(const char* url) {
    if (g_localPort > 0) {
        char* r = InternalHttpsGet(url, TRUE, NULL);
        if (r) return r;
    }
...
int BuildDNSQueryHTTPS(const char* domain, unsigned char* buf) {
    int p = 0;
    buf[p++] = 0; buf[p++] = 1; // ID=1 (Match Go)
    buf[p++] = 1; buf[p++] = 0; // RD=1
    buf[p++] = 0; buf[p++] = 1; // QD=1
    buf[p++] = 0; buf[p++] = 0; 
    buf[p++] = 0; buf[p++] = 0; 
    buf[p++] = 0; buf[p++] = 0; 
    ...
    while ((dot = strchr(ptr, '.'))) {
        int l = (int)(dot - ptr); buf[p++] = l; memcpy(buf+p, ptr, l); p += l; ptr = dot + 1;
    }
    if (*ptr) { int l = strlen(ptr); buf[p++] = l; memcpy(buf+p, ptr, l); p += l; }
    buf[p++] = 0;
    buf[p++] = 0; buf[p++] = 65; // TYPE=65
...
char* FetchECHFromDoH(const char* dohUrl, const char* sni) {
    ...
    // Skip Question
    while (p < rLen && resp[p] != 0) p += resp[p] + 1;
    p += 5; 
    ...
    for (int i = 0; i < anC; i++) {
        ...
        if (type == 65) {
            ...
            int rp = 2; 
            // Skip Priority + TargetName
            if (rp < rdLen && rdata[rp] == 0) rp++;
            else { while (rp < rdLen && rdata[rp] != 0) rp += rdata[rp] + 1; rp++; }
            
            while (rp + 4 <= rdLen) {
                int k = (rdata[rp] << 8) | rdata[rp+1];
                int vl = (rdata[rp+2] << 8) | rdata[rp+3];
                rp += 4;
                if (rp + vl > rdLen) break;
                if (k == 5) {
                    char* out = (char*)malloc(vl * 2); // safe size
...
BOOL IsWindows8OrGreater() {
    return GetProcAddress(GetModuleHandleW(L"kernel32.dll"), "SetProcessMitigationPolicy") != NULL;
}

void SetSystemProxy(BOOL enable) {
    if (enable && g_localPort <= 0) return;
    wchar_t server[64], bypass[] = L"<local>";
    _snwprintf(server, 64, L"127.0.0.1:%d", g_localPort);
    
    if (IsWindows8OrGreater()) {
        HKEY k;
        if (RegCreateKeyExW(HKEY_CURRENT_USER, REG_PATH_PROXY, 0, NULL, 0, KEY_SET_VALUE, NULL, &k, NULL) == 0) {
            DWORD en = enable ? 1 : 0;
            RegSetValueExW(k, L"ProxyEnable", 0, REG_DWORD, (BYTE*)&en, 4);
            if (enable) {
                RegSetValueExW(k, L"ProxyServer", 0, REG_SZ, (BYTE*)server, (wcslen(server)+1)*2);
                RegSetValueExW(k, L"ProxyOverride", 0, REG_SZ, (BYTE*)bypass, (wcslen(bypass)+1)*2);
            }
            RegCloseKey(k);
        }
    } else {
        ...
        opts[2].Value.pszValue = bypass;
        
        INTERNET_PER_CONN_OPTION_LISTW list = {sizeof(list), NULL, 3, 0, opts};
...
BOOL IsSystemProxyEnabled() {
    HKEY k; DWORD en = 0, sz = 4;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, REG_PATH_PROXY, 0, KEY_READ, &k) == 0) {
        RegQueryValueExW(k, L"ProxyEnable", NULL, NULL, (BYTE*)&en, &sz);
        RegCloseKey(k);
    }
    return en == 1;
}
