...
static int frag_write(BIO *b, const char *in, int inl) {
    ...
    // [Logic] 如果 ECH 开启，强制跳过 Padding 注入，保护 ClientHello 完整性
...
BOOL CALLBACK InitCryptoCallback(PINIT_ONCE InitOnce, PVOID Parameter, PVOID *Context) {
    ...
    // [Fix] 显式配置 TLS 1.3 加密套件 (OpenSSL 1.1.1+/3.0 规范)
    // ECH 依赖 TLS 1.3，如果未正确设置，ClientHello 可能不包含 ECH 所需的算法，导致 illegal_parameter
...
int tls_init_connect(TLSContext *ctx, const char* target_sni, const char* target_host, const CryptoSettings* settings) {
    ...
    // [Fix] 策略调整：如果启用 ECH，完全跳过挂载 BIO_f_fragment
    // 即使在 BIO 内部禁用了 Padding，BIO 本身的分片行为或内存拷贝可能破坏 ECH ClientHello 的结构
    // 或者触发 OpenSSL 在处理复杂扩展时的 bug。直连 Socket 能够排除这些干扰。
